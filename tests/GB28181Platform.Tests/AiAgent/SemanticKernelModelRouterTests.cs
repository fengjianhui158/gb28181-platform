...
public class SemanticKernelModelRouterTests
{
    ...
    [Fact]
    public void FromConfiguration_FallsBackToLegacyQwenApiSettings()
    {
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["QwenApi:BaseUrl"] = "https://api.deepseek.com",
                ["QwenApi:ApiKey"] = "legacy-key",
        ...
        Assert.Equal("https://api.deepseek.com", options.Text.BaseUrl);
        Assert.Equal("legacy-key", options.Text.ApiKey);
        Assert.Equal("deepseek-chat", options.Text.Model);
        Assert.Equal("https://api.deepseek.com", options.Vision.BaseUrl);
        Assert.Equal("legacy-key", options.Vision.ApiKey);
        Assert.Equal("deepseek-chat", options.Vision.Model);
    }
}
