...
```
tests/
└── GB28181Platform.Tests/
    ├── GB28181Platform.Tests.csproj      # CREATE: xUnit + NSubstitute
    ├── Helpers/
    │   └── MockDbHelper.cs              # CREATE: SqlSugar Mock 工具
    ├── Diagnostic/
    │   └── DiagnosticEngineTests.cs     # CREATE: 诊断引擎流水线测试
    └── AiAgent/
        ├── FunctionRegistryTests.cs     # CREATE: 函数注册表测试
        └── AiAgentServiceTests.cs       # CREATE: AI Agent 服务测试
```
...
## Task 13: 创建测试项目 + 补全实体字段

**Files:**
- Create: `tests/GB28181Platform.Tests/GB28181Platform.Tests.csproj`
- Create: `tests/GB28181Platform.Tests/Helpers/MockDbHelper.cs`
- Modify: `GB28181Platform.sln` (添加测试项目)
- Modify: `src/GB28181Platform.Domain/Entities/DiagnosticLog.cs` (补全 Task 7 引用的字段)

- [ ] **Step 1: 补全 DiagnosticLog 实体字段**

Task 7 的 DiagnosticEngine 代码设置了 `StepType` 和 `ScreenshotPath`，但当前实体缺少这两个属性。在 `DiagnosticLog.cs` 的 `DurationMs` 属性后添加：
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    <ProjectReference Include="..\..\src\GB28181Platform.Domain\GB28181Platform.Domain.csproj" />
    <ProjectReference Include="..\..\src\GB28181Platform.Diagnostic\GB28181Platform.Diagnostic.csproj" />
    <ProjectReference Include="..\..\src\GB28181Platform.AiAgent\GB28181Platform.AiAgent.csproj" />
  </ItemGroup>
...
Run: `dotnet sln GB28181Platform.sln add tests/GB28181Platform.Tests/GB28181Platform.Tests.csproj`
Expected: `Project ... added to the solution.`

- [ ] **Step 4: 创建 MockDbHelper**
...
public static class MockDbHelper
{
    /// <summary>
    /// 创建诊断引擎测试用的 Mock DB。
    /// 模拟 Queryable&lt;Device&gt;, Updateable&lt;DiagnosticTask&gt;, Insertable&lt;DiagnosticLog&gt;。
    /// </summary>
    public static ISqlSugarClient CreateForDiagnostic(Device? device)
    {
        var db = Substitute.For<ISqlSugarClient>();

        // Mock Queryable<Device>.FirstAsync(...)
        ...
        // Mock Updateable<DiagnosticTask> fluent chain
    ...
    /// <summary>
    /// 创建 AI Agent 测试用的 Mock DB。
    /// 模拟 Insertable&lt;AiConversation&gt; (保存对话记录)。
    /// </summary>
...
```bash
git add tests/ GB28181Platform.sln src/GB28181Platform.Domain/Entities/DiagnosticLog.cs
git commit -m "test: add xUnit test project with NSubstitute and fix DiagnosticLog entity"
```
...
**Files:**
- Create: `tests/GB28181Platform.Tests/Diagnostic/DiagnosticEngineTests.cs`

**测试覆盖点:**
| 测试方法 | 验证行为 |
|---------|---------|
| `ExecutesStepsInStepTypeOrder` | 步骤按 Ping→Port→Browser 排序执行，即使传入顺序乱序 |
| `StopsWhenContinueNextIsFalse` | Ping 失败(ContinueNext=false)时，后续步骤不执行 |
| `DeviceNotFound_NoStepsExecuted` | 设备不存在时提前返回，不执行任何步骤 |

- [ ] **Step 1: 编写 DiagnosticEngineTests**
...
public class DiagnosticEngineTests
{
    ...
    public async Task RunDiagnostic_ExecutesStepsInStepTypeOrder()
    {
        ...
        // Assert — 应按 Ping → Port → Browser 顺序执行
    ...
    public async Task RunDiagnostic_DeviceNotFound_NoStepsExecuted()
    {
        ...
        var db = MockDbHelper.CreateForDiagnostic(null); // 设备不存在
...
Run: `dotnet test tests/GB28181Platform.Tests/ --filter "FullyQualifiedName~DiagnosticEngineTests" -v normal`
Expected: 3 tests passed.

- [ ] **Step 3: Commit**

```bash
git add tests/GB28181Platform.Tests/Diagnostic/
git commit -m "test(diagnostic): add unit tests for diagnostic engine pipeline"
```

---

## Task 15: AI Agent 单元测试

**Files:**
- Create: `tests/GB28181Platform.Tests/AiAgent/FunctionRegistryTests.cs`
- Create: `tests/GB28181Platform.Tests/AiAgent/AiAgentServiceTests.cs`

**测试覆盖点:**
| 测试类 | 测试方法 | 验证行为 |
|-------|---------|---------|
| FunctionRegistryTests | `Get_ReturnsRegisteredFunction` | 注册后可通过 Name 查找 |
| FunctionRegistryTests | `Get_ReturnsNull_WhenNotRegistered` | 未注册函数返回 null |
| FunctionRegistryTests | `GetDefinitions_ReturnsAll` | 获取全部函数定义 |
| AiAgentServiceTests | `ReturnsContent_WhenNoFunctionCall` | Qwen 直接回答时返回内容 |
| AiAgentServiceTests | `ExecutesFunctionCall_ThenReturnsFinalAnswer` | Function Calling 循环正确执行 |
| AiAgentServiceTests | `EnforcesMaxFunctionCallsLimit` | 超过 5 次函数调用后强制终止 |

- [ ] **Step 1: 编写 FunctionRegistryTests**
...
public class AiAgentServiceTests
{
    ...
    {
        // Arrange — Qwen 直接返回文本，无 Function Call
    ...
    {
        // Arrange — 第一次 Qwen 返回 function_call，第二次返回最终回答
        var callCount = 0;
        var qwen = Substitute.For<IQwenClient>();
        qwen.ChatAsync(
                Arg.Any<List<ChatMessage>>(),
    ...
    {
        // Arrange — Qwen 持续返回 function_call，模拟死循环
        var qwen = Substitute.For<IQwenClient>();
        qwen.ChatAsync(
                Arg.Any<List<ChatMessage>>(),
                Arg.Any<List<FunctionDefinition>>())
        ...
        // Assert — MaxFunctionCalls=5，超出后返回限制提示
...
Run: `dotnet test tests/GB28181Platform.Tests/ -v normal`
Expected: 9 tests passed (3 DiagnosticEngine + 3 FunctionRegistry + 3 AiAgentService).

- [ ] **Step 4: Commit**

```bash
git add tests/GB28181Platform.Tests/AiAgent/
git commit -m "test(ai): add unit tests for FunctionRegistry and AiAgentService"
```

---

## Task 16: 科技风改造诊断面板 + AI 问答页
...
| .NET 8 降级 | Task 1 |
| 科技风 UI | Task 2 (主题) + Task 3 (设备列表) + Task 16 (诊断+AI) |
| 单元测试 | Task 13 (测试项目) + Task 14 (诊断引擎) + Task 15 (AI Agent) |
| 非功能: 100-500路 | 架构支撑，ZLMediaKit 单机数千路 |

### 类型一致性检查

- `IDiagnosticStep` 在 Task 7 定义，Task 8/9 实现，Task 14 测试 — 签名一致 ✅
- `IDiagnosticEngine` 在 Task 7 定义，Task 10 注入，Task 14 测试 — 一致 ✅
- `IQwenClient` 在 Task 11 定义，Task 12 使用，Task 15 测试 — 一致 ✅
- `IAgentFunction` 在 Task 11 定义，Task 12 注册，Task 15 测试 — 一致 ✅
- `FunctionRegistry` 在 Task 11 定义，Task 12 注入，Task 15 测试 — 一致 ✅
- `IZlmClient` 在 Task 4 定义，Task 5 使用 — 一致 ✅
- `PlayResult` 在 Task 5 定义并返回 — 一致 ✅
- `DiagnosticLog.StepType/ScreenshotPath` 在 Task 13 补全，Task 7 引用 — 一致 ✅
